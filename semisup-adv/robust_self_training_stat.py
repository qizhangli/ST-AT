...
from losses_stat import stat_loss
...
parser.add_argument('--resume-opt', default=None, type=str)
parser.add_argument('--resume-epoch', default=None, type=int)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        train_metrics.append(dict(
            ...
            entropy_loss_unlabeled=0))

        # print progress
        if batch_idx % args.log_interval == 0:
            logging.info(
                'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tCE_UPPER: {:.4f}\tCE_CLEAN: {:.4f}\tCE_LOWER: {:.4f}'.format(
...
def main():
    ...
    if args.resume:
        ...
        start_epoch = 1 + args.resume_epoch
    else:
        start_epoch = 1
